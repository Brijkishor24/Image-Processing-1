...
def invert_effect(decode_array_to_img):
    invert_eff=cv2.bitwise_not(decode_array_to_img)
    status,output_img=cv2.imencode('.PNG',invert_eff)
    return output_img

def HDR_effect(decode_array_to_img):
    hdr_eff=cv2.detailEnhance(decode_array_to_img,sigma_s=12,sigma_r=0.15)
    status,output_img=cv2.imencode('.PNG',hdr_eff)
    return output_img
# Code ends
